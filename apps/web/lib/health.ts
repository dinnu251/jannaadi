// Shared health-check logic — each dependency actually probed, never assumed (B12).
// db: SELECT 1. pubsub: topic exists. gemini: metadata fetch of the pinned model.
// 200 when all ok, 503 when any fails. Exposed at BOTH /healthz and /api/healthz:
// /healthz is the reserved health-check path of the external HTTPS load balancer and
// never reaches the container, so /api/healthz is the canonical, reliably-routed path.
